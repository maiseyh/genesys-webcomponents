...
import { GuxRadialProgressAccent } from './gux-radial-progress.types';
...
export class GuxRadialProgress {
  ...
  /**
   * Set the color of the progress element
   */
  @Prop()
  accent: GuxRadialProgressAccent = 'neutral';
  ...
  render(): JSX.Element {
    return this.canShowPercentage(this.value, this.max)
      ? this.showPercentageState(this.value, this.max, this.accent)
      : this.showSpinnerState();
  }

  private canShowPercentage(value: number, max: number): boolean {
  ...
  private showPercentageState(
    value: number,
    max: number,
    accent: GuxRadialProgressAccent
  ): JSX.Element {
    return (
      <div
        class={`gux-${accent}`}
